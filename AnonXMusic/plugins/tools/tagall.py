...
@app.on_message(command(["@all","تاك للكل"])) 
async def mentionall(client, message):
    chat_id = message.chat.id
    if message.chat.type == "private":
        return await message.reply("عزيزي هذا الأمࢪ فقط في المجموعات!!")
    ...
    try:
        ...
        if participant.status in ("administrator", "creator"):
            is_admin = True
    if not is_admin:
        return await message.reply("أنت لست آدمن، هذا الأمࢪ فقط للآدمنية!!")

    if message.reply_to_message and message.text:
        return await message.reply("أرسل تاك للكل أو @all مع الرسالة")
    elif message.text:
        mode = "text_on_cmd"
        msg = message.text
...
@app.on_message(command(["الغاء التاك", "ايقاف"]))
async def cancel_spam(client, message):
    if not message.chat.id in spam_chats:
        return await message.reply("عزيزي التاك متوقف حالياً!!")
    is_admin = False
    try:
        ...
        if participant.status in ("administrator", "creator"):
            is_admin = True
    if not is_admin:
        return await message.reply("أنت عضو، وهذا الأمر فقط للآدمنية!!")
    else:
        try:
            spam_chats.remove(message.chat.id)
        except:
            pass
        return await message.reply("تم إيقاف التاك بنجاح عزيزي 🥀")
